...
保護者に、管理アプリURL、管理スプレッドシートURL、編集用フォームURLは共有しません。
...
役員用の入口は、スプレッドシートの表ではなく、管理アプリ画面です。
...
管理アプリでは、申込一覧、会員確定、取下げ、会員名簿を扱います。
...
5. HTMLファイル `Index` を作り、`src/Index.html` を貼る。
6. フォームURLをGoogleサイトの `FORM_URL_HERE` と差し替える。
7. テスト申込を送る。
8. 管理アプリで会員確定できるか確認する。
